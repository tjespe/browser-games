...
c.author}catch(x){}n=q=!0;d.get(b.comment+"?id="+h.id+"&com="+encodeURIComponent(c.comment)+"&author="+encodeURIComponent(c.author)).then(function(a){n=q=!1;c.comment="";c.refresh()})["catch"](function(){alert("Your message was not submitted.\nPlease try again.");q=n=!1})}else l.commentForm.$setTouched()};c.ifDisabled=function(){return q};g.$on("$locationChangeStart",function(a){q&&(a.preventDefault(),alert(l.master.locChangeAlert))});p.onbeforeunload=function(){if(q)return locChangeAlert};c.goBack=
function(){l.master.routeChanged?p.history.back(1):e.path("/")};c.scrollDown=function(){$("html, body").animate({scrollTop:$(document).height()},1E3)};c.jquery=function(){var a=$("textarea").outerHeight();$("textarea").keyup(function(){var b=$("form.com-container>div").height()-24;$("a.glyphicon[type=submit]").css("line-height",b+"px");b=$("textarea").width()*(30/270);b=Math.floor($("textarea").val().length/b)+1;$("textarea").css("height",b*a+"px");l.view.ifMobile||$("html, body").animate({scrollTop:$(document).height()})});
