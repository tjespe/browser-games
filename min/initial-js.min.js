...
c.allGamesAreDisplayed()||(r=!0,a=t.getGames+"?from="+c.games.length+"&amount="+a+"&d="+Math.floor(Date.now()/36E5),0<q.pass.length&&(a+="&pass="+q.pass),p.get(a,1400).then(function(a){Array.prototype.push.apply(c.games,a.data);r=!1})["catch"](function(a,b){console.log(a,b);r=!1}))};c.rate=function(a,b){};c.like=function(a){c.rate(a,"like",1)};c.dislike=function(a){c.rate(a,"dislike",-1)};c.ifDisabled=function(){return c.disabled};c.locChangeAlert="Please don't leave the page until the server has responded";
...
p.get("src/js/scripts/analytics.js",0).then(function(a){eval(a)});p.get("src/js/objects/"+(c.norsk?"no":"en")+"-text.json").then(function(a){c.textData=a})["catch"](function(a,b){console.log(a,b);confirm("Unable to load textdata, do you want to reload the page?")?location.reload(!0):null})}]);app.controller("categoryCtrl",["$http","$routeParams","$scope","urls","initialJSON","$lhttp",function(a,f,d,g,e,h){var b=this,m=!1;b.tag=f.category;b.amount=1;b.noGames=!1;d.master.loc="Thorin-Games \u2014 "+b.tag;d.master.desc=d.master.norsk?"Here you can find all our games with the tag "+b.tag+" sorted by popularity":'Her kan du finne alle v\u00e5re spill med taggen "'+b.tag+'" sortert etter popularitet';b.games=b.tag in d.master.categoryGames?b.games=d.master.categoryGames[b.tag]:[];h.get(g.countGames+
"?tag="+b.tag+"&d="+Date.now()+"&pass="+e.pass,800).then(function(a){b.amount=Number(a);var e=d.master.desc,f=d.master.norsk?" games":" spill";d.master.desc=e.substring(0,e.indexOf(f))+" "+a+e.substring(e.indexOf(f));18<b.amount?b.requestGames(18):b.requestGames(b.amount);0==b.amount&&(b.noGames=!0)});b.requestGames=function(a){if(!m&&!b.allGamesAreDisplayed()){m=!0;var f=b.games.length;f+a>b.amount&&(a=b.amount-(f+a));a=g.getGames+"?from="+f+"&amount="+a+"&tag="+b.tag+"&d="+Date.now()+"&pass="+e.pass;
h.get(a,1500).then(function(a){Array.prototype.push.apply(b.games,a.data);m=!1;d.master.categoryGames[b.tag]=b.games})}};b.allGamesAreDisplayed=function(){return b.games.length<b.amount}}]);app.controller("gameCtrl",["$scope","$routeParams","$http","$sce","$interval","$timeout","$q","$window","$rootScope","$location","initialJSON","$lhttp","urls",function(a,f,d,g,e,h,b,m,k,q,p,t,c){function r(){/^[\d.,]+%$/.test(a.detail.height)&&p.jquery.then(function(){$("embed").ready(function(){h(function(){$("embed").attr("height",$("embed").width()*(Number(a.detail.height.split("%")[0])/100)+1)},1E3)})})}var n=!1,l=!1,u=0,v=b.defer();"https:"==location.protocol&&(location.protocol="http:");t.get(c.getGames+
...
"?d="+Math.floor(Date.now()/1E4)+"&id="+f.id+"&coms="+a.detail.comments.length,b=d({method:"get",url:b,timeout:v.promise});b.success(function(b){n=!1;u=0;JSON.parse(b)&&a.refresh();for(b=0;b<a.detail.comments.length;b++)a.detail.comments[b].date||(a.detail.comments[b].date=1459870175813),a.detail.comments[b].date+=diff/1E3});b.error(function(a,b){n=!1})}};var x=e(a.ifAnyChanges,2400);a.$on("$destroy",function(){return e.cancel(x)});try{a.author=localStorage.username}catch(w){a.author=""}a.comment=
"";a.submit=function(){if(!l&&0<a.comment.length&&0<a.author.length){a.commentForm.$setUntouched();try{localStorage.username=a.author}catch(w){}n=l=!0;d.get(c.comment+"?id="+f.id+"&com="+a.comment+"&author="+a.author).then(function(b){n=l=!1;a.detail.comments=b.data;a.comment="";a.refresh()})["catch"](function(){alert("Your message was not submitted.\nPlease try again.");l=n=!1})}else a.commentForm.$setTouched()};a.ifDisabled=function(){return l};k.$on("$locationChangeStart",function(b){l&&(b.preventDefault(),
