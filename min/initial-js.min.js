...
app.service("initialJSON",["$http","$lhttp","urls",function(a,f,d){this.pass=encodeURIComponent(window.location.search.slice(1)+window.location.hash.slice(1));this.json=f.get(this.pass.length?"https://script.google.com/macros/s/AKfycbzEVUBnYRqGOFS6309I9Oe748omXUWLjpDScrjYatNvxKuL6BEU/exec?pass="+this.pass:d.initialJSON);this.jquery=f.get("https://code.jquery.com/jquery-2.2.3.min.js",0);this.jquery.then(function(a){eval(a)})}]);
app.directive("script",function(){return{restrict:"E",scope:!1,link:function(a,f,d){"text/javascript-lazy"===d.type&&eval(f[0].text)}}});
